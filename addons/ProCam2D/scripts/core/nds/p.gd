...
export(AxisConstraint) var constraint_axis = AxisConstraint.Y

var _path: Path2D setget set_path
...
func _ready():
	connect("child_entered_tree", self, "update_configuration_warning")
	_update_path()
...
func apply_constraint(camera_pos: Vector2) -> Vector2:
	if not _path and Engine.editor_hint:
		return camera_pos
